...
create or replace function public.requesting_user_id() returns text language sql stable as $$
select nullif(
...
insert into channel_types (type, name, color, character_limit)
values ('TWITTER', 'Twitter / X', '#000000', 280),
    ...
    ('TIKTOK', 'Tiktok', '#000000', 100) on conflict (type) do nothing;
-- 2. USER CHANNELS
...
create policy user_channels_policy on user_channels for all using (user_id = requesting_user_id()) with check (user_id = requesting_user_id());
...
insert into idea_groups (name)
values ('Unassigned'),
    ('To Do'),
    ('In Progress'),
    ('Done') on conflict (name) do nothing;
-- IDEAS
...
create policy ideas_policy on ideas for all using (user_id = requesting_user_id()) with check (user_id = requesting_user_id());
-- SCHEDULED POSTS
create table if not exists scheduled_posts (
    ...
    status text not null default 'draft' check (
        status in ('queue', 'draft', 'published', 'failed')
    ),
...
create policy scheduled_posts_policy on scheduled_posts for all using (user_id = requesting_user_id()) with check (user_id = requesting_user_id());