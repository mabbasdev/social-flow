...
insert into public.channel_types (type, name, color, character_limit)
values
...
create policy user_channels_policy on public.user_channels
	for all
...
insert into public.idea_groups (name)
values ('Unassigned'), ('To Do'), ('In Progress'), ('Done')
...
create table if not exists public.ideas (
	id uuid primary key default gen_random_uuid(),
	user_id text not null,
	group_id uuid not null references public.idea_groups(id) on delete restrict,
	title text not null,
	description text,
...
create policy ideas_policy on public.ideas
	for all
	using (user_id = public.requesting_user_id())
	with check (user_id = public.requesting_user_id());
...
create policy scheduled_posts_policy on public.scheduled_posts
	for all
	using (user_id = public.requesting_user_id())
	with check (user_id = public.requesting_user_id());
